...
class ProxyController():
    def __init__(self, name=None, port=None, denied_port=None, authenticated=False):
        if not name:
            self.address = ''
        else:
            self.name = name
    ...
    def remove(self):
        if self.address:
            ...
            utils.remove_running_container(self.name)
            self._remove_config()
    ...
    def _remove_config(self):
        logging.info(f'Removing Config for Proxy Server {self.name}')
